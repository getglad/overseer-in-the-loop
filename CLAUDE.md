...
2. **HITL is tool-level, not agent-level.** Approval happens inside the tool function body (e.g. `hitl_current_datetime` in `src/loop/agent.py`), before the real work executes. NAT's ReAct agent has no pre-tool-call hook — wrapping at the tool level is the supported pattern.
